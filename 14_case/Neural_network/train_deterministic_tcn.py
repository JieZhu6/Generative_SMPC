...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="SMPC scenario-tree instances per gradient update",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--objective-cost-scale", type=float, default=4e4, metavar="COST",
        help="positive fixed horizon-cost divisor in monetary units",
    )
    parser.add_argument(
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    if args.learning_rate <= 0 or args.objective_cost_scale <= 0:
        raise ValueError("learning rate and objective cost scale must be positive")
...
def main() -> None:
    ...
    if not (
        ...
        raise ValueError(
            "Deterministic TCN training requires the 22-state fixed-PV DECS "

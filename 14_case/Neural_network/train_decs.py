...
The IEEE-14 mapping is ``rho[27] -> chi[22]``, where
...
def main() -> None:
    ...
    parser.add_argument(
        "--data", type=Path,
        default=ROOT / "Data_generation" / "data" / "e2e14_decs_pgm_fixedpv_N10000",
        help="22-state fixed-PV DECS dataset generated by native PGM",
    )
    parser.add_argument(
        "--output", type=Path,
        default=Path(__file__).resolve().parent / "decs_pgm_fixedpv.pt",
    )
    parser.add_argument("--hidden-dims", type=int, nargs=2, default=(64, 64), metavar=("H1", "H2"))
    ...
    if not (
        data_metadata.get("bus_type_model") == "fixed_PV_PQ"
        and data_metadata.get("enforce_q_limits") is False
        and data_metadata.get("pv_to_pq_switching") is False
        and int(data_metadata.get("chi_dim", -1)) == 22
    ):
        raise ValueError("DECS data must contain 22-state fixed-PV labels without PV/PQ switching")

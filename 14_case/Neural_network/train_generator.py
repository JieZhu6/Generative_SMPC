...
def diversity_loss(
    ...
    trajectory : torch.Tensor, shape (batch, candidates, horizon, free_dim)
        ...
        candidate's last two dimensions are flattened for the squared L2
        distance in equation (31).
    ...
    kernel = torch.exp(-distance2 / (2.0 * sigma**2))
                ...
                if args.max_grad_norm > 0.0:
                    ...
                    for name, parameter in model.named_parameters():
                        if parameter.grad is not None and not torch.isfinite(parameter.grad).all():
                            raise FloatingPointError(
                                f"non-finite generator gradient in {name} at batch {batch_number}"
                            )
                optimizer.step()
...
def build_parser(benchmark: str = "csng") -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--data", type=Path,
        default=ROOT / "Data_generation" / "data" / f"e2e14_N{N_train}_S{S_num}_T{T_horizon}",
    ...
    parser.add_argument(
        ...
        help="external SMPC instances per gradient update",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stage2-learning-rate", type=float, default=1e-4,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "cost scale near 4e4 this gives a raw-cost coefficient near 2.5e-7"
        ),
    )
    ...
    parser.add_argument(
        "--economic-warmup-epochs", type=int, default=10,
    ...
    if diversity_enabled:
        parser.add_argument(
            ...
            help="Gaussian diversity width in normalized free-variable space",
        )
    else:
    ...
    parser.add_argument(
        "--economic-min-delta", type=float, default=5.0, metavar="COST",
        help=(
...
def main(benchmark: str = "csng") -> None:
    ...
    if not (
        ...
        raise ValueError(
            "Generator training requires the 22-state fixed-PV DECS without "
            "PV-to-PQ switching"
        )
    ...
    print(
        ...
        f"loads=(B,{datasets['train'].n_scenarios},{datasets['train'].horizon},11,2)"
    )
    print(

...
def main() -> None:
    ...
    parser.add_argument(
        "--forecast-deviation", type=float, default=0.10, metavar="D",
        help="tanh预测误差相对日内曲线的最大幅度，取值范围[0,1)",
    )
    parser.add_argument(
        "--load-scale-min", type=float, default=0.8, metavar="LOWER",
        help="节点负荷相对PGLib基准值的保守非对称绝对下界",
    )
    parser.add_argument(
        "--load-scale-max", type=float, default=1.1, metavar="UPPER",

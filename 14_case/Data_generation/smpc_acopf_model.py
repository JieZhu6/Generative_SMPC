"""PGLib IEEE 14 节点系统的两阶段场景型 SMPC--ACOPF 模型。
...
def branch_states(case: Case14, vm: np.ndarray, va: np.ndarray) -> dict[str, np.ndarray]:
    """由节点电压计算每条支路两端的功率和电流幅值。

    参数
    ----
    case : Case14
        IEEE 14 节点网络数据，提供支路连接关系和两端复导纳。
...
def nodal_injections(case: Case14, vm: np.ndarray, va: np.ndarray):

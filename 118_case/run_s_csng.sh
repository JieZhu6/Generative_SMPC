#!/bin/bash
#SBATCH --job-name=s-csng
#SBATCH --partition=gpu
#SBATCH --gpus=1
#SBATCH --ntasks=1
#SBATCH --output=/data/home/scvk217/run/118_case/118_case/slurm_logs/s-csng-%j.out

# Train S-CSNG on one Slurm-assigned GPU.
set -euo pipefail
...
OUTPUT_DIR="$PROJECT_DIR/output/remote5090/s_csng"
...
GPU_MONITOR_LOG="$LOG_DIR/gpu-s-csng-${SLURM_JOB_ID}.csv"
nvidia-smi \
...
python -u Neural_network/train_generator_s_csng.py \
    --data "$DATA_DIR" \
    --decs "$DECS_PATH" \
    --output "$OUTPUT_DIR/generator_tcn_s_csng.pt" \
    --stage1-output "$OUTPUT_DIR/generator_tcn_s_csng_stage1.pt" \
    --epochs 700 \
    --stage1-epochs 80 \
    ...
    --device cuda \
    --seed 2026 \
    2>&1 | tee "$LOG_DIR/train-s-csng-${SLURM_JOB_ID}.log"

...
def generation_cost(case: Case118, pg: np.ndarray) -> float:
...
def solve_exact_power_flow(
    ...
    ----------
    net : pandapowerNet
        Reusable IEEE-118 pandapower network.
    case : Case118
...
def validate_shared_schedule(
    ...
    case : Case118
        IEEE-118 topology, costs, and operating limits.
    current_load : np.ndarray, shape (n_load_buses, 2)
    ...
    free_dim = (
        len(case.nonreference_active_generators)
        + len(case.voltage_control_buses)
    )
    if schedule.shape != (horizon, free_dim):

...
def validation_predictions(
    ...
    """Predict one complete DECS split in bounded memory.
    ...
    split : {"validation", "test"}
        Dataset split evaluated; validation preserves the historical default.
    ...
    indices = np.load(data_dir / "split" / f"{split}_indices.npy")
    ...
    print(f"{split} inference: {len(indices)} samples on {device}", flush=True)
    with torch.no_grad():
        for start in range(0, len(indices), batch_size):
            ...
            print(
                f"\r{split} inference: {completed}/{len(indices)} "
...
def training_domain(
    ...
    case : Case118
        IEEE-118 network data defining nominal loads and free-variable bounds.
...
def main() -> None:
    ...
    parser.add_argument(
        "--data", type=Path,
        default=ROOT / "Data_generation" / "data" / f"e2e118_decs_pandapower_N{n_sample}",
    ...
    parser.add_argument(
        ...
        help="dataset split used for constraint-decision evaluation",
    )
    parser.add_argument(
        "--output", type=Path,
    ...
    result = validation_predictions(
        args.data, completion, args.batch_size, device, split=args.split,
    )

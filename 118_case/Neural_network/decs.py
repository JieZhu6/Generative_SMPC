"""Differentiable fixed-PV equality completion for the IEEE 118-bus case.
...
class EqualityCompletionSurrogate(nn.Module):
    ...
    def forward(self, rho_normalized: torch.Tensor) -> torch.Tensor:
        ...
        Parameters
        ----------
        rho_normalized : torch.Tensor, shape (..., input_dim)
...
class ACReconstruction(nn.Module):
    ...
    def __init__(self):
        """Load the local PGLib IEEE-118 topology as fixed tensor buffers."""
        super().__init__()
        case = load_case118()
        real = torch.float32
    ...
    def free_variable_bounds(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns
        -------
        lower, upper : tuple[torch.Tensor, torch.Tensor], shape (batch_size, free_dim)
    ...
    def full_load(self, load: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Parameters
        ----------
        load : torch.Tensor, shape (batch, n_load_buses, 2)
            Active and reactive net loads ``[P,Q]`` in MW/Mvar.

        Returns
        -------
        pd, qd : tuple[torch.Tensor, torch.Tensor], shape (batch, n_bus)
    ...
    def generator_active_power(self, u: torch.Tensor) -> torch.Tensor:
        ...
        Parameters
        ----------
        u : torch.Tensor, shape (batch, free_dim)
            Free variables with active powers in MW and voltages in p.u.

        Returns
        -------
        torch.Tensor, shape (batch, n_gen)
    ...
    def specification(self, u: torch.Tensor, load: torch.Tensor) -> torch.Tensor:
        """Construct the fixed-PV PF specification ``rho``.
        ...
        Returns
        -------
        torch.Tensor, shape (batch, rho_dim)
    ...
    def _specification_from_full_load(
        ...
        pd, qd : torch.Tensor, shape (batch, n_bus)
            Full-bus active and reactive loads in MW/Mvar.
    ...
    def voltage_state(self, u: torch.Tensor, chi: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        chi : torch.Tensor, shape (batch, chi_dim)
            ``[theta_PV, theta_PQ, V_PQ]`` in rad and p.u.

        Returns
        -------
        vm, va : tuple[torch.Tensor, torch.Tensor], shape (batch, n_bus)
    ...
    def _nodal_injection_from_branch_power(
        ...
        Parameters
        ----------
        vm : torch.Tensor, shape (batch, n_bus)
        ...
        Returns
        -------
        p, q : tuple[torch.Tensor, torch.Tensor], shape (batch, n_bus)
    ...
    def reconstruct(
        ...
        rho : torch.Tensor or None, shape (batch, rho_dim), default=None
        ...
        -------
        dict[str, torch.Tensor]
            Full voltage state, generator powers, branch flows, and the
...
class DifferentiableEqualityCompletion(nn.Module):
    ...
    rho_mean, rho_std : torch.Tensor, shape (rho_dim,)
        Training-set input mean and standard deviation.
    chi_mean, chi_std : torch.Tensor, shape (chi_dim,)
...
def load_decs_checkpoint(path: Path, device: torch.device) -> DifferentiableEqualityCompletion:
    ...
    expected_input_dim = physics.rho_dim
    expected_output_dim = physics.chi_dim
    checkpoint_case = checkpoint.get("data_metadata", {}).get("case_name")
    if checkpoint_case != "pglib_opf_case118_ieee":
        raise ValueError(
            f"DECS checkpoint case={checkpoint_case!r}; IEEE-118 is required. "
            "Regenerate the dataset and retrain DECS."
        )
    if (
        int(checkpoint["input_dim"]) != expected_input_dim
        or int(checkpoint["output_dim"]) != expected_output_dim
    ):
        raise ValueError(
            "DECS checkpoint dimensions "
            f"{checkpoint['input_dim']}->{checkpoint['output_dim']} are incompatible; "
            f"IEEE-118 fixed-PV reconstruction requires "
            f"{expected_input_dim}->{expected_output_dim}. "

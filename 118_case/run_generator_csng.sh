#!/bin/bash
#SBATCH --job-name=csng
#SBATCH --partition=gpu
#SBATCH --gpus=1
#SBATCH --ntasks=1
#SBATCH --output=/data/home/scvk217/run/118_case/118_case/slurm_logs/csng-%j.out

# Train the standard CSNG generator on one Slurm-assigned GPU.
set -euo pipefail
...
OUTPUT_DIR="$PROJECT_DIR/output/remote5090/csng"
LOG_DIR="$PROJECT_DIR/slurm_logs"

# Match the S-CSNG experiment: micro-batch 12 and one backward pass per update.
...
GPU_MONITOR_LOG="$LOG_DIR/gpu-csng-${SLURM_JOB_ID}.csv"
nvidia-smi \
...
python -u Neural_network/train_generator.py \
    --data "$DATA_DIR" \
    --decs "$DECS_PATH" \
    --output "$OUTPUT_DIR/generator_tcn_clip.pt" \
    --stage1-output "$OUTPUT_DIR/generator_tcn_clip_stage1.pt" \
    --epochs 700 \
    --stage1-epochs 80 \
    ...
    --device cuda \
    --seed 2026 \
    2>&1 | tee "$LOG_DIR/train-csng-${SLURM_JOB_ID}.log"

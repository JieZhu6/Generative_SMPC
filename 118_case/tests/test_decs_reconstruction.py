...
def dense_nodal_injection_reference(
    ...
    ----------
    physics : ACReconstruction
        IEEE-118 network tensors and base-MVA scaling.
...
class ACReconstructionEquivalenceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Create deterministic random IEEE-118 voltage states."""
    ...
    def test_sparse_nodal_values_match_dense_ybus(self) -> None:
        ...
        for sparse_value, dense_value in zip(sparse, dense):
            # The 118-bus network has strong branch-flow cancellation;
            # 0.01 MW/Mvar is the observed float32 accumulation floor.
            self.assertTrue(torch.allclose(
                sparse_value, dense_value, rtol=3e-6, atol=1e-2,
            ))
